...
class Notifications extends React.PureComponent {
    ...
    render() {
        let messages = this.props.listNotifications.map((x) =>
            <NotificationItem key={x.id} type={x.type} value={x.value} html={x.html} markAsRead={() => this.props.markNotificationAsRead(x.id)} />
        );
        ...
        let notifs = (
            ...
            <button aria-label="Close" onClick={this.props.handleHideDrawer} style={{position: "absolute", top: 15, right: 0, visibility: "hidden", cursor: "pointer"}}><img alt="close" src={ex} style={{visibility: "visible", height: "15px", width: "15px"}}/>
            </button>
            <p>
